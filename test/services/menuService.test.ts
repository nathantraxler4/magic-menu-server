...
let mockCreate = openai.chat.completions.create as jest.Mock;
...
describe('generateMenu', () => {
    ...
    describe('WHEN calling generate recipe with one recipe', () => {
        test('THEN a menu object is returned', async () => {
            mockCreate.mockImplementation(() => Promise.resolve({ choices: [{ message: { content: '["some description1"]' } }] }))

            const menu = await menuService.generateMenu([recipes[0]]);
    
            expect(menu).toMatchObject({"backgroundImage": 1, "courses": [{"description": "some description1", "name": "name1"}]});
    ...
    describe('WHEN calling generate recipe with 3 recipes and LLM responds with 3 descriptions.', () => {
        test('THEN an error is thrown', async () => {
            mockCreate.mockImplementation(() => Promise.resolve({ choices: [{ message: { content: '["some description1", "some description2", "some description3"]' } }] }))
            const menu = await menuService.generateMenu(recipes);
            
            expect(menu).toMatchObject({"backgroundImage": 1, "courses": [{"description": "some description1", "name": "name1"}, {"description": "some description2", "name": "name2"}, {"description": "some description3", "name": "name3"}]});
            expect(mockCreate).toHaveBeenCalled();
        });
    })
